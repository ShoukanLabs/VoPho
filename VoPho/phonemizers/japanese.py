...
tens = ['', 'じゅう', 'にじゅう', 'さんじゅう', 'よんじゅう', 'ごじゅう', 'ろくじゅう', 'ななじゅう', 'はちじゅう',
        'きゅうじゅう']
hundreds = ['', 'ひゃく', 'にひゃく', 'さんびゃく', 'よんひゃく', 'ごひゃく', 'ろっぴゃく', 'ななひゃく', 'はっぴゃく',
            'きゅうひゃく']
...
def replace_tashdid_2(s):
    vowels = set('aieoɯ0123456789.?!_。؟？！．．．＠@＃#＄$％%＾^＆&＊*（)(）_+=[「」]></\\`~～―ー∺"')
    result = []
    i = 0
    while i < len(s):
...
def replace_tashdid(input_string):
    ...
    while i < len(input_string):
        if i + 1 < len(input_string) and input_string[i] == input_string[i + 1] and input_string[i] not in set('aiueo'):
            result.append('ʔ')
...
def process_japanese_text(ml):
    ...
    small_char_map = str.maketrans("ぁぃぅぇぉ", "あいうえお")
    ml = ml.translate(small_char_map)
...
class Phonemizer:
    ...
    def _process_japanese_text(self, text):
        # Convert text to uppercase and apply transformations from 'pre'
        text = text.upper()
        for a, b in pre:
            text = text.replace(a, b)

        # Convert to romaji, apply transformations, and convert to lowercase
        text = self.katsu.romaji(text, capitalize=False).lower()

        # Apply specific sound replacements with regex or standard replace
        text = re.sub(r'j', 'dʑ', text)
        text = re.sub(r'tt|t t|ʔt', 'ʔt', text)
        text = text.replace('ssh', 'ɕɕ')

        # Convert numbers in string (assuming this function is already optimized)
        text = convert_numbers_in_string(text)

        # Apply Roma2IPA transformations
        for a, b in roma_mapper:
            text = text.replace(a, b)

        # Simplify nasal sounds replacement with a single regex
        text = re.sub(r'n(?![aeio])', 'ɴ', text)

        # Handle special cases using regex lookaheads and lookbehinds
        text = re.sub(r'(?<=[brk])j(?=o)', 'ʲ', text)
        text = replace_tashdid_2(text)

        # Handle vowel lengthening for repeated characters
        text = re.sub(r'([aeio])\1', r'\1ː', text)

        # Apply nasal sound replacements from nasal_sounds list
        for a, b in nasal_sounds:
            text = text.replace(a, b)

        # Specific sound and phrase replacements
        text = text.replace(' ɴ', 'ɴ')
        text = text.replace(' neɽitai ', 'naɽitai')
        text = text.replace('harɯdʑisama', 'arɯdʑisama')
        text = re.sub(r'(?<!\s)ki ni ɕinai', ' ki ni ɕinai', text)
        text = re.sub(r'(?<!\s)de aɽoɯ', ' de aɽoɯ', text)

        # Remove leading spaces and return the processed text
        return text.lstrip()

    def phonemize(self, text):
        # Process the text for Japanese phonemization
        output = self._process_japanese_text(text)

        # Preserve the trailing space if the original text ends with one
        if text.endswith(' '):
            output += ' '

        # Apply k_mapper substitutions using a single regex pass
        for a, b in k_mapper:
            output = re.sub(re.escape(a), b, output)

        # Perform key replacements in bulk using regex
        output = re.sub(r'y', 'j', output)
        output = re.sub(r'ɯa', 'wa', output)

        # Replace repeating 'a' patterns using regex
        output = replace_repeating_a(output)  # Assuming this is already optimized

        # Simplify spacing around '~' using regex
        output = re.sub(r'\s+~', '~', output)

        # Apply phonetic patterns efficiently with regex
        phonetic_patterns = [
            (r'oː~o oː~ o', 'oː~~~~~~'),
            (r'aː~aː', 'aː~~~'),
            (r'oɴ naː', 'onnaː'),
        ...
        # Random space fix using regex patterns from the spaces list
        for a, b in spaces:
            output = re.sub(re.escape(a), b, output)

        # Random symbol fix with space handling using regex
        for a, b in symbols:
            output = re.sub(re.escape(a), f' {b} ', output)
